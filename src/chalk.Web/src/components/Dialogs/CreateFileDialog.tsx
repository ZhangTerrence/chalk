...
import { useCreateFileForModuleMutation } from "@/redux/services/file.ts";
...
export const CreateFileDialog = () => {
  ...
  const [createFileForModule, { isLoading, isSuccess }] = useCreateFileForModuleMutation();
  ...
  const onSubmit = async (data: CreateFileType) => {
    ...
    await createFileForModule({ entityId: module.id, data: formData }).unwrap();
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex min-w-80 flex-col gap-y-4">
          ...
          <div className="flex flex-col gap-y-2 grow">
            <Label htmlFor="resource">Attachment</Label>
